...
const DEFAULT_IMG = "http://books.google.com/books/content?id=YjaatAEACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api"
...
function post(book, append = false) {
  return storageService.post(BOOKS_KEY, book, append)
}
...
function saveCache(cache) {
  storageService.save(CACHE_KEY, cache)
}
...
function getNearBooksIds(bookId) {
  ...
  return storageService.query(BOOKS_KEY)
    .then(books => {
      ...
      return {
        prev: (idx <= 0) ? books[books.length - 1].id : books[idx - 1].id,
        next: (idx >= books.length - 1) ? books[0].id : books[idx + 1].id
      }
    })
...
function getBooks(searchKey) {
  return new Promise((resolve, reject) => {
    if (searchCache[searchKey]) {
      ...
      resolve(searchCache[searchKey])
      console.log('cache');

    } else {

    const url = `https://www.googleapis.com/books/v1/volumes?printType=books&q=${encodeURI(searchKey)}`

    axios.get(url)
      .then(({ data }) => {
        console.log('server');
        searchCache[searchKey] = _getAdjustedBooksData(data.items)
...
function _createCache() {
    storageService.query(CACHE_KEY).then(data => {
      searchCache =  data || {}
    })
}

function _getAdjustedBooksData(data) {
  return data.map(

    ({ id, volumeInfo: { title, authors, subtitle, publishedDate,
      description, pageCount, categories, imageLinks, language } }) => (

      {
        ...
        thumbnail: (imageLinks) ? imageLinks.thumbnail : DEFAULT_IMG,
        language,
        listPrice: {
          "amount": utilService.getRandomIntInclusive(15, 350),
